...
import type { MembershipApplication } from '@saccosphere/schemas'

type AdminApplicationItem = MembershipApplication & {
  submitted_at?: string
  status?: string
  ref?: string
  ...
  return (
    <div className="p-5">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg px-2.5 py-2 mb-4 text-sm text-blue-800 border-l-4 border-l-blue-500">
        Review and approve or reject submitted membership applications.
      </div>
      ...
      {isLoading ? (
        ...
        (applications ?? []).map((app: AdminApplicationItem) => (
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex gap-3 items-center">
                ...
                <div>
                  ...
                  <div className="text-xs text-ink-muted">Submitted via Saccosphere · {app.submitted_at ? new Date(app.submitted_at).toLocaleDateString('en-KE') : '—'}</div>
                </div>
              </div>
              <span
                className={`${app.status === 'submitted' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'} px-2.5 py-0.5 rounded-full text-[11px] font-semibold`}
              >
