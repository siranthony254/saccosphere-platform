...
export function Overview() {
  ...
  return (
    <div className="p-5">
      {overviewLoading ? (
        <div className="p-6 text-ink-muted">Loading platform data...</div>
      ) : overviewError || !overview ? (
        <div className="p-6">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-800">
            <div className="font-semibold mb-1">Failed to load overview</div>
            <div className="text-[13px] text-red-900">
              {overviewError && typeof overviewError === 'object' && 'message' in overviewError
                ? String((overviewError as any).message)
                : 'Unable to fetch platform data. Please check your connection.'}
            </div>
          </div>
        </div>
      ) : (
        <>
          <PageHeader
            ...
            actions={
              ...
              >
                ...
                <span>{overview.all_systems_operational ? 'All systems operational' : 'System issues detected'}</span>
              </div>
            }
          />
          ...
          <div className="grid grid-cols-4 gap-3 mb-5">
            <MetricCard
              ...
              delta={overview.transaction_volume_change_pct
            ...
            <MetricCard
              ...
              delta="Registered & active"
            />
            <MetricCard
              label="Total members"
              value={overview.total_members.toLocaleString()}
              delta="Approved memberships"
            />
            <MetricCard
                  ...
                  <>
                    ...
                    <div className="flex justify-between text-[9px] text-ink-faint mt-2">
                      {(revenue ?? []).slice(-12).map((row: any) => (
                        <span key={row.month}>{String(row.month).slice(-2)}</span>
                      ))}
                    </div>
                  </>
                )}
              ...
              <Card
                ...
                action={
                  <div className="flex items-center gap-1 text-[11px] text-violet-500">
                    ...
                    <span>{connected ? 'Real-time' : 'Connecting...'}</span>
                  </div>
                }
              >
                <DataTable
                  columns={[
                    ...
                    { key: 'member', header: 'Member', render: (row: any) => <span>{row.member}</span> },
                    { key: 'sacco', header: 'SACCO', render: (row: any) => <span className="text-ink-muted">{row.sacco}</span> },
                    { key: 'type', header: 'Type', render: (row: any) => <span>{row.type}</span> },
                    {
                      key: 'amount',
                      header: 'Amount',
                      render: (row: any) => (
